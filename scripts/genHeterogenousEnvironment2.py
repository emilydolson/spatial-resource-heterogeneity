...
parser.add_argument("--backgroundOn", action="store_true", help="Don't put any resources in the desert")
...
parser.add_argument("--randRadius", action="store_true", help = "Sets radius of each patch to a random integer between 6 and 25.")
...
parser.add_argument("--environmentPicker", action="store_true", help = "Use the environmentPicker GUI to select cells to fill with resources")
...
gradResources = ["resNOT", "resAND", "resOR", "resNOR", "resNAND", "resORN", "resANDN", "resXOR", "resEQU"] #types of resources available in gradients
...
def main():
    ...
    background = False #is there always a background quantity of all resources
    if args.backgroundOn:
        background = True
    ...
    if args.environmentPicker:
        ...
        cells = ep.cell_picker().run()
        print cells
        cells = list(set(cells))
        print "final length", len(cells)
        print cells
        cells.sort()
        for resource in gradResources:
    ...
    ent = calcEntropy(randResources, anchors, rads)
...
def genRandResources():
    ...
    """
    randResources = []
    nEach = args.nPatches / len(gradResources)
    extras = args.nPatches % len(gradResources)
...
def genGradient(resource, inflow, radius, loc, common=True):
    ...
    """
    return "GRADIENT_RESOURCE " + str(resource) + ":height=" + str(radius) + ":plateau=" + str(inflow) +":spread=" + str(radius-1) + ":common=" + str(int(common)) + ":updatestep=1000000:peakx=" + str(loc[0]) + ":peaky=" + str(loc[1]) + ":plateau_inflow=" + str(inflow) + ":initial=" + str(inflow) + "\n"
...
def genCell(resource, cells):
    return "CELL " + resource + ":" + ",".join([str(i) for i in cells]) + ":inflow=10000:outflow=.001:initial=1000\n"
...
def entropyHistogramRandomPlacements(resList):
    """
    Generates a graph of the average entropy for an environment generated at each distance (where a distance refers to the number 
    of cells between the anchor points of each pair of neighboring circles) for the specified radius (int) and list of resources to 
    include in the environment (list of strings indicating resource names with numeric characters appended to uniquely identify each
    ...
    errs = []
    ents = []
    for i in range(500): #Loop over distances
        args.nPatches = args.patchesPerSide**2
        if args.randNPatches:
            args.nPatches += random.randrange(-60,60)
        anchors = calcRandomAnchors(args.boundedAnchors)
        resList = genRandResources()
        random.shuffle(resList) #rearrange resources
        ents.append(calcEntropy(resList, anchors, [random.randrange(6,35) for i in range(len(anchors))], False))
        
    plt.hist(ents)
    plt.savefig("randomEntropies" + str(args.patchRadius)+ ".eps")
...
def graphEntropyVsDistance(radius, resList):
    ...
    results = []
    errs = []
    for i in range(35): #Loop over distances
        ents = []
        anchors = calcTightAnchors(i, args.patchesPerSide)
        for j in range(100): #run 30 replicates
            random.shuffle(resList) #rearrange resources
            ents.append(calcEntropy(resList, anchors, radius, False))
            #print radius, anchors
        entAvg = float(sum(ents))/len(ents)
        results.append(entAvg)
        errs.append(np.std(ents))
...
def graphEntropyVsRadius(resList, anchors):
    ...
    for i in range(60): #Loop over radii
        ents = []
        for j in range(100): #Do 30 replicates at each radius
            random.shuffle(resList) #randomize resource placement
            ents.append(calcEntropy(resList, anchors, i, False))
        entAvg = float(sum(ents))/len(ents)
        results.append(entAvg)
        errs.append(np.std(ents))

    plt.errorbar(range(len(results)), results, yerr=errs)
    plt.savefig("entropyvsradius_errors.png")
    print results, errs
    ...
    for i in range(args.worldSize):
        for j in range(args.worldSize):
            for k in range(len(anchors)):
                #print k, len(anchors), len(rad), rad
